...
class _DirectChatPageState extends State<DirectChatPage> {
  ...
  String get _chatId => 'direct:${widget.matchId}';
      ...
      if (token != null && token.isNotEmpty) {
        try {
          ...
          if (parts.length == 3) {
            ...
            final jwtUid = decoded['user_id']?.toString() ??
          ...
          debugPrint('[DIRECT CHAT] JWT decode error: $e');
        }
      }
    }
  ...
  Future<void> _loadHistory() async {
    ...
    try {
      final token = await _storage.readToken();
      final msgs =
          await _chatApi.getDirectChat(widget.matchId, token: token);
      if (mounted) {
        for (final m in msgs) {
  ...
  void _connectWs() async {
    try {
      ...
      final wsUrl = _chatApi.getDirectWebSocketUrl(widget.matchId);
      ...
      _wsSub = channel.stream.listen(
        (data) {
          try {
            ...
            if (msgType == 'typing') {
              final typerId =
                  (msg['sender_id'] ?? msg['user_id'] ?? '').toString();
              if (typerId.isEmpty || typerId == _myUserId) return;
              final typerName =
                  (msg['sender_name'] ?? msg['name'] ?? '').toString().trim();
            ...
            if (msgType == 'system') return; // Ignore system msgs in direct chat
            ...
            if (wsSenderId == _myUserId) return;

            // Foreign message — notify the provider
            if (mounted) {
              Provider.of<ChatNotificationProvider>(context, listen: false)
                  .addNotification(
                chatId: _chatId,
                chatName: widget.partnerName,
                senderName: wsSenderId.isNotEmpty
                    ? widget.partnerName
                    : 'Seseorang',
                message: wsContent,
              );
  ...
  Map<String, dynamic> _normalizeWsMessage(Map<String, dynamic> msg) {
    return {
      'id': (msg['id'] ?? '').toString(),
      'sender_id': (msg['sender_id'] ?? msg['user_id'] ?? msg['from'] ?? '')
          .toString(),
      'sender_name':
                        ...
                        ? Center(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  'Mulai percakapan dengan ${widget.partnerName}!',
                                  style: TextStyle(
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Text(
            '${widget.partnerName} sedang mengetik...',
